...
async function getMessages(matchId, limit = 50) {
  const db = await getDb();
  if (!db) return [];
  return db.select().from(messages).where(eq(messages.matchId, matchId)).limit(limit);
}
async function markMessagesAsRead(matchId, receiverId) {
  const db = await getDb();
  if (!db) return;
  await db.update(messages).set({ read: 1 }).where(and(eq(messages.matchId, matchId), eq(messages.receiverId, receiverId)));
}
...
var appRouter = router({
  ...
  profile: router({
    ...
    update: protectedProcedure.input(
      z2.object({
        ...
        lookingFor: z2.enum(["male", "female", "both"]).optional(),
        location: z2.string().optional(),
        photoUrl: z2.string().optional(),
        interests: z2.string().optional()
      })
  ...
  discover: router({
    browse: protectedProcedure.input(
      z2.object({
        lookingFor: z2.enum(["male", "female", "both"]),
        limit: z2.number().optional().default(20)
      })
    ).query(async ({ ctx, input }) => {
      return await getProfilesByGender(ctx.user.id, input.lookingFor, input.limit);
    }),
    like: protectedProcedure.input(z2.object({ targetUserId: z2.number() })).mutation(async ({ ctx, input }) => {
  ...
  matches: router({
    ...
    updateStatus: protectedProcedure.input(z2.object({ matchId: z2.number(), status: z2.enum(["liked", "matched", "blocked"]) })).mutation(async ({ ctx, input }) => {
      await updateMatchStatus(input.matchId, input.status);
      return { success: true };
    })
  }),
  messages: router({
    send: protectedProcedure.input(z2.object({ matchId: z2.number(), receiverId: z2.number(), content: z2.string() })).mutation(async ({ ctx, input }) => {
      await sendMessage(input.matchId, ctx.user.id, input.receiverId, input.content);
      return { success: true };
    }),
    getConversation: protectedProcedure.input(z2.object({ matchId: z2.number(), limit: z2.number().optional().default(50) })).query(async ({ ctx, input }) => {
